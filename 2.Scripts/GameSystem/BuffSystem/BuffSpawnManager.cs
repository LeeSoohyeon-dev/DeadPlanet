...
public class BuffSpawnManager : MonoBehaviour
{
	...
	private void SpawnInitialBuffs()
	{
		for (int i = 0; i < initialBuffCount; i++)
		{
			...
			if (IsValidSpawnPosition(spawnPosition))
			{
				...
				spawnedBuffPositions.Add(spawnPosition);
			}
		}
	}
	...
	private Vector3 GetRandomPositionAroundPlayer()
	{
		...
		for (int i = 0; i < 50; i++)
		{
			...
			{
				Vector3 spawnPos = new Vector3(hit.position.x, 0f, hit.position.z);
	...
	private bool IsValidSpawnPosition(Vector3 position)
	{
		...
		checkPos = new Vector3(navMeshPos.x, 0f, navMeshPos.z);
		...
		if (player != null)
		{
			...
			float distance2D = Vector2.Distance(playerPos2D, checkPos2D);

			if (distance2D < minDistanceFromPlayer)
				return false;
		}

		if (IsTooCloseToEnemies(checkPos))
			return false;

		if (IsTooCloseToOtherBuffs(checkPos))
			return false;
