...
public class GameManager : MonoBehaviour
{
    ...
    // 승리/게임오버 중 하나가 확정되면 다른 결과가 화면을 덮어쓰지 못하게 하는 래치
    ...
    private void OnSceneLoaded(Scene scene, LoadSceneMode mode)
    {
        ...
        {
            if (!hasProcessedStageScene && UI.instance != null)
            {
            ...
            GameEvents.RaisePlaySound(SoundType.BGM);
        }
    }
    ...
    {
        if (isProcessingSequence || hasGameEnded)
            return;
        ...
        try
        {
            ...
            GameEvents.RaiseSlowMotion(1.5f);
            GameEvents.RaisePlaySound(soundType);

            await uiTask;
            GameEvents.RaisePauseTime();
        }
        catch (OperationCanceledException)
        {
