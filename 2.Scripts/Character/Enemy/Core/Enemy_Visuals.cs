...
public class Enemy_Visuals : MonoBehaviour
{
    ...
    {
        // 보조 무기는 원거리 적 전용 — 다른 타입이면 없음
        Enemy_Range rangeEnemy = GetComponentInParent<Enemy_Range>();
        if (rangeEnemy == null)
            return null;

        Enemy_SeconoderyRangeWeaponModel[] weaponModels = GetComponentsInChildren<Enemy_SeconoderyRangeWeaponModel>(true);
        Enemy_RangeWeaponType weaponType = rangeEnemy.weaponType;
    ...
    private async UniTaskVoid RetrySetupRandomWeaponAsync()
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            if (currentWeaponModel != null)
            {
                ...
                // Enemy_Range.Start()의 SetupWeapon()은 이 시점 전에 이미 실패했으므로
                // 모델이 확보된 지금 다시 호출해 gunPoint를 연결한다
                GetComponent<Enemy_Range>()?.SetupWeapon();
                return;
            }
        }
