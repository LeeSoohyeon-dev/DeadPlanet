...
public class Enemy_Boss : Enemy
{
    ...
    {
        Transform point = impactPoint != null ? impactPoint : transform;

        MassDamage(point.position, impactRadius, jumpAttackDamage);
    }

    private void MassDamage(Vector3 impactCenter, float impactRadius, int damage)
    {
        HashSet<GameObject> uniqueEntities = new HashSet<GameObject>();
        Collider[] colliders = Physics.OverlapSphere(impactCenter, impactRadius, ~whatIsAlly);

        bool hasDamagedAnyone = false;
        ...
        foreach (Collider hit in colliders)
        {
            ...
            ApplyPhysicalForceTo(impactCenter, impactRadius, hit);
        }

        if (hasDamagedAnyone)
            GameEvents.RaisePlaySound(SoundType.EnemyJumpImpact);
    }
