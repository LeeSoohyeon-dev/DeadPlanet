...
public class AudioManager : MonoBehaviour
{
    ...
    private readonly List<SoundType> soundsToStopBuffer = new List<SoundType>();
    private readonly List<SoundType> soundsToRemoveBuffer = new List<SoundType>();
    ...
    private void StopLowerPrioritySounds(int priority)
    {
        ...
        foreach (SoundType soundType in currentlyPlaying)
        {
            ...
            {
                soundsToStopBuffer.Add(soundType);
            }
        }

        for (int i = 0; i < soundsToStopBuffer.Count; i++)
        {
            StopAudio(soundsToStopBuffer[i]);
        }
    }
    ...
    private void UpdateCurrentlyPlaying()
    {
        ...
        foreach (SoundType soundType in currentlyPlaying)
        {
            ...
            {
                soundsToRemoveBuffer.Add(soundType);
            }
        }

        for (int i = 0; i < soundsToRemoveBuffer.Count; i++)
        {
            currentlyPlaying.Remove(soundsToRemoveBuffer[i]);
        }
    }
}
